...
`internal/drag.ts` is the scaffold around a pointer drag, and it is here for what the five copies had already drifted into rather than for the repetition. Sidebar, Panes, WindowPane, DataTable and ScrollZone all drag something; the arithmetic differs in every one and stays where it is. What was written five times is the three listeners, the `data-dragging` mark, the document's text selection taken and given back, and a teardown an unmount can call — and the DataTable copy was missing the last two, so dragging a column boundary in Safari selected the text of every cell the pointer crossed and a table that unmounted mid-drag left the selection taken away for good. `release` and `onEnd` are deliberately two things: an unmount has to run the first and must not run the second, because a component that disappeared did not finish resizing. There is no `requestAnimationFrame` in it — a `pointermove` is already delivered once per frame, so coalescing again would only add latency. ScrollZone is the one that does **not** use it, and that is deliberate too: it takes the pointer and the selection at its drag _threshold_ rather than at the press, because a press on a strip is far more often a click on a card inside it.

`internal/observe.ts` is one `ResizeObserver` and one `IntersectionObserver` per threshold, for the whole page. Eleven components measure themselves and eleven `Animate*` wrappers watch for their own element arriving on screen, and each of the twenty-two was building an observer of its own — a dashboard of eight charts inside a PageLayout with a Panes and a ScrollZone in it had a dozen registrations the browser walked on every layout, for no reason at all. An observer takes any number of targets; what the module adds is the routing from a target back to the callbacks watching it, and the `unobserve` when the last of them goes. The threshold is the one thing an `IntersectionObserver` cannot vary per target, which is why that one is keyed by it.

`internal/search.ts` is the third rule of that kind. Three components let a reader type at a list of their own — DataTable, CommandPalette and Transfer — and each had a `matches` of its own; they disagreed about accents, which is not a preference anybody chose. It is also where the fixed cost of typing lives: `String.prototype.normalize` is expensive, and a `matches(item, query)` signature quietly asks for both sides to be folded on every comparison, which is what put a `normalize` on every cell of every row of a DataTable on every character typed. So the API folds a haystack once and a needle once, and all three components memoise the haystacks on the rows rather than on the query.

`internal/media.ts` and `internal/link.ts` are two rules the library makes rather than two pieces of machinery it shares. `media.ts` holds one `MediaQueryList` per query string, for a reason `useSyncExternalStore` makes easy to miss: `getSnapshot` is called on every render _and_ every commit of every component that subscribes, so building the list inside it makes a fresh one each time — a PageLayout with a Header, a Footer, a Sidebar and a SidebarTrigger asks the same question five times per render, and a page of `Animate*` wrappers asks it once each. A `MediaQueryList` is live, so one per query is all there ever needs to be. `usePrefersReducedMotion` lives here rather than in `animate.tsx` so a Carousel and a ScrollZone can ask without pulling the eleven effects in with the answer. `link.ts` is `safeRel`, which merges `noopener noreferrer` into whatever `rel` a caller wrote whenever a link opens somewhere other than this tab — TextLink, a Menu row and a NavigationMenu link all take a `target`, and a library where only one of the three is safe is a library whose users cannot tell which. It is a merge and not a default because the usual reason to write a `rel` by hand is `nofollow`, which is an SEO decision and must not silently take the protection off.
...
- **`display`** — it shows something and nothing more. Typography, TextLink, Blockquote, Highlight, Divider, Chip, Badge, Avatar, AppLogo, Icon, Shortcut, List, DataList, Table, CodeBlock, Timeline, Breadcrumb, Anchor, TreeView.
...
- **A chromium run loses its browser partway through the suite, and it is not ours.** It reads either as `Failed to run the test <file>` / `[vitest] Browser connection was closed while running tests` or, on Windows, as a file that prints `(0 test)` and then never says anything again. **Read the counts before chasing the component.** The run exits 1 on that one unhandled error, every test that got to start passed, and the file count comes up short only because the files still queued behind the dead browser never run at all — `Test Files 113 passed (137)` over `Tests 2453 passed (2453)` is this and not a regression. It is chromium-only and has now been seen on all three runners, twice in one run, so "never on macOS" is no longer true; it has landed on nine different files — ProgressCircular, AspectRatio, AreaChart, AnimateTyping, Meter, Container, AnimateGrow, AnimateSlide, AnimateZoom — so no single test is guilty. What they share is a position, and the position is not a coincidence. Vitest's sequencer runs the **larger files first** when it has no cache to sort by, which a CI checkout never has, so a run there is ordered by size descending — and all nine are in its last quarter, positions 106 to 132 of the hundred and thirty-seven, none of them over 4.7 kB. That is the stretch where the files are small and the harness is tearing down and rebuilding a tester frame twice a second. A local run with a warm cache sorts by duration rather than size, so the tail there is a different set of files — which is worth knowing before reading anything into _which_ file a local run names. Vitest's own tracker has the same report open against browser mode. Do not chase it through the component that happened to be running, and do not paper over it with a retry — a retry would also swallow the real failures this suite exists to catch. The timeouts above are the mitigation: the run fails in twenty minutes instead of hanging, and a re-run passes.

  **The rate scales with the file count, and it stopped being occasional.** At a hundred and twenty-six files it was rare enough to be a curiosity; at a hundred and thirty-seven it fires on most local runs and a clean one takes two or three attempts. Read the counts before you go looking for what you broke: `Tests 2453 passed (2453)` beside a short _file_ count is this and nothing else — no test that started has ever failed to it.

  One thing here **can** cause it, and it is worth knowing which: an uncaught render error. A component that throws during `render()` with nothing above it to catch takes the page down, and the page is the tester frame every file in this suite shares — so the _next_ file in the queue is the one that reports a closed browser connection, and the file that actually threw passes. `useConfirm`'s "no provider" test hit exactly this. Test a throw behind an error boundary, which is also how a consumer meets it.
