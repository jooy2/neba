...
- **The rest of `internal/`** — the arithmetic, the tables, the glyphs, `sizer.tsx`, `picker.tsx`. A module with no directive belongs to whichever graph imports it, which is exactly right for these; only the nine that hold a context, an effect or a store (`animate.ts`, `avatar-group.ts`, `button-group.ts`, `media.ts`, `menu.ts`, `page-layout.ts`, `bottom-navigation.ts`, `calendar.tsx`, `chart-frame.tsx`) are marked.
...
`internal/media.ts` and `internal/link.ts` are two rules the library makes rather than two pieces of machinery it shares. `media.ts` holds one `MediaQueryList` per query string, for a reason `useSyncExternalStore` makes easy to miss: `getSnapshot` is called on every render _and_ every commit of every component that subscribes, so building the list inside it makes a fresh one each time — a PageLayout with a Header, a Footer, a Sidebar and a SidebarTrigger asks the same question five times per render, and a page of `Animate*` wrappers asks it once each. A `MediaQueryList` is live, so one per query is all there ever needs to be. `usePrefersReducedMotion` lives here rather than in `animate.ts` so a Carousel and a ScrollZone can ask without pulling the eleven effects in with the answer. `link.ts` is `safeRel`, which merges `noopener noreferrer` into whatever `rel` a caller wrote whenever a link opens somewhere other than this tab — TextLink, a Menu row and a NavigationMenu link all take a `target`, and a library where only one of the three is safe is a library whose users cannot tell which. It is a merge and not a default because the usual reason to write a `rel` by hand is `nofollow`, which is an SEO decision and must not silently take the protection off.
