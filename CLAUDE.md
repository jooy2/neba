...
4. When a component is added, document it under `docs/` — see [Documentation](#documentation) for the six places that means. That is also what makes it viewable alongside the others.
...
- **The rest of `internal/`** — the arithmetic, the tables, the glyphs, `sizer.tsx`, `picker.tsx`, `notch.tsx`. A module with no directive belongs to whichever graph imports it, which is exactly right for these; only the thirteen that hold a context, an effect or a store (`animate.tsx`, `bottom-navigation.ts`, `button-group.ts`, `calendar.tsx`, `chart-frame.tsx`, `defaults.ts`, `drop.ts`, `fieldset.ts`, `media.ts`, `menu.ts`, `page-layout.ts`, `run.tsx`, `screen.ts`) are marked.
...
- **No `transform` on a control, ever.** Scaling resamples the label, and text that shifts under the cursor is what reads as cheap. State changes are expressed in colour and depth only. The _indicator inside_ a control is the exception, and it is a closed list of four — a Switch's thumb on `inset-inline-start`, a Checkbox's tick on `stroke-dashoffset`, a Radio's dot on `width`/`height`, a Rating's fill on `width`. Each carries no text, each is the whole of what the state says, and none of them scales. The one thing in the library that _does_ scale is a `Gallery` tile's photograph under `hover="zoom"`, which is opt-in: the frame around it is `overflow: hidden` and never moves, and a photograph has no label to resample. An Image's `rotate` and `flip` are the other transforms a photograph takes, and they are not motion: the caller sets them once, a quarter turn or a mirror resamples nothing, and both sit on the individual `rotate` and `scale` properties so that `transform` stays free for the zoom. A `float` label is the one piece of _text_ that travels, and it is opt-in too: it moves on `top` and `inset-inline-start` and resizes on `font-size`, so it is set again at every size rather than scaled, and it moves when the focus lands or a value arrives, never under a passing pointer.
...
- **A notched field's edge is drawn by the notch, not by the shell.** A label on the top edge (`labelPlacement` `notch` or `float`) needs a gap in the border, the plate's top light and the focus ring at once, and none of them can be cut. So the shell takes `fieldSheetClasses` — the sheet with its edge taken off, the `outline` border kept at 1px and made transparent so nothing inside moves — and `internal/notch.tsx` draws the edge in pieces around the label. A notched shell must therefore leave `fieldRingClasses` off, or it draws a second, uncut ring. With no label there is nothing to cut, and the shell keeps its own edge.
...
- **Never `outline-none`.** Tailwind v4 routes outline style through `--tw-outline-style`, which `outline-none` zeroes — killing the focus ring. Use the shorthand, and declare the ring at rest so the focus has a colour to move: `[outline:0_solid_transparent] outline-offset-2 focus-visible:[outline:2px_solid_var(--n-ring)]` — the focused half keeps the whole shorthand, since a host's `:focus-visible { outline: auto }` is one selector and a class alone would be decided against it by generation order. What travels is `outline-color` and never `outline-width`: a browser rounds an outline to a whole device pixel, so a width going 0 → 2px is a full 1px ring from the first frame, a dead interval, and a snap. The resting half is in `transitionClasses` itself (`ringRestClasses`), because `outline-color` initialises to `currentColor` and the thirty-odd elements that declare only the focused half would otherwise fade their ring in from their own text colour. An `[outline:none]` beside it is the same mistake in the other spelling — two `outline` declarations of equal specificity, decided by generation order. **On a field's shell the ring is flush** (`fieldRingClasses`, offset 0) because the hairline under it is already the ring's colour; everywhere else it keeps the offset, because a flush ring on a filled control sits on a fill of its own family.
...
- **Adding a colour family is four edits.** One entry in `NebaColor`; the five base tokens in each of the three theme blocks in `src/styles.css`; the thirteen derived tokens in the derived block, which mix those five with `color-mix()`; and the family's entry in `Alert`'s `rolesFor`, which the type checker demands.

### Shared prop vocabulary

`src/types.ts` holds the vocabulary every component draws from: `NebaSize`, `NebaColor`, `NebaDensity`, `NebaVariant`, `NebaElevation`, `NebaOrientation`, `NebaSide`, `NebaAlign`, `NebaLabelPlacement`, and the `NebaStyleProps` bundle. A `size` of `md` must mean the same thing on every component. **Do not invent a second spelling for an idea that already has one** — see [docs/ko/design/prop-conventions.md](docs/ko/design/prop-conventions.md).
...
| `chart.ts`, `chart-frame.tsx`, `chart-line.tsx`, `chart-brush.tsx` | Chart arithmetic, everything that is not a mark, the line marks, and the window on a long series |
...
| `media.ts`, `link.ts` | One `MediaQueryList` per query, and safe `rel` and `href` |
| `inert.ts` | `inert` on React 18 and 19 |
| `keys.ts` | What a key combination means |
| `highlight.ts` | The one module that loads, for CodeBlock |
...
The line it draws is the load-bearing part, and it is the same one `styleSlots()` draws for colour: **a responsive value is one that fits in an inline `--n-*` slot, never one that is a class name.** Tailwind only ever sees literal class names, so a per-breakpoint class map would be five complete ladders in the bundle of every page that draws the component — which is why `span`, `offset`, `columns`, `spacing`, `rowSpacing`, `columnSpacing`, `maxWidth` and `direction` take a map and `size`, `variant`, `color` and `elevation` deliberately do not. The CSS half is one cascade per slot name in `styles.css`, four media blocks each, and they cannot be generated; that cost is what keeps the list of responsive axes short on purpose.
...
`internal/wheel.ts` is the other gesture two components had to answer the same way, and it is the smallest module in the folder for a reason worth stating: what it holds is a rule, not machinery. A mouse has one wheel and it points down the page, which is the one axis a horizontal strip does not run along — so a ScrollZone's rail and a Tabs bar are the two things on a page a mouse cannot reach into, and a third would have been next. `bindAxisWheel` turns a wheel rolled over the element into travel along its horizontal scroll, and **holds it at the ends**: handing the wheel back the moment the strip runs out is what makes the page lurch mid-flick, because the reader was still pushing the strip and what answered was the article behind it. Moving the pointer off it is the way out, which is why nothing here watches for one.

Four details in it are load-bearing. It is a **native** listener rather than React's `onWheel`, because React attaches that one passively and a passive listener cannot `preventDefault` — which is the whole gesture. It takes nothing at all on an element that **does not overflow**, measured on each event rather than read off the last render, and that guard is what makes it safe to leave on by default: three tabs in a row are not a scroll container as far as the reader is concerned and must not behave like one. And it deliberately answers only the wheel, leaving a Ctrl-held zoom and a sideways trackpad swipe alone; the swipe already scrolls the strip and the browser does it better, so **`overscroll-behavior: contain` is what holds that one at the ends** — which is why every caller sets it, and why a vertical ScrollZone needs the class rather than the handler. The fourth is the `defaultPrevented` guard at the top: the event bubbles, so a strip inside a strip — or a NumberField being scrubbed on one — would otherwise have its notch answered twice and travel twice as far as the hand asked for.

The one place the two callers disagree is the default, and the disagreement is the point: `ScrollZone`'s `wheel` is off and `Tabs`' is on, because a ScrollZone draws a pair of buttons for the pointer that has no other way along and a tab bar draws none.
...
`internal/color.ts` is the arithmetic ColorPicker needs — three representations, the conversions between them, one parser and one formatter. It is under four hundred lines, and it is the reason a colour library is not a third runtime dependency. HSV is the model the panel is drawn in and it never leaves: round-tripping through RGB would lose the hue of every greyscale colour, and the rail would snap to red the moment the pointer reached a corner.

#### `internal/notch.tsx`

`internal/notch.tsx` is the label on a field's top edge, and it is here because ten components draw a field shell and all of them take `labelPlacement`: TextField, NumberField, Select, Combobox and the picker shell's six. What it draws is the edge itself, in three pieces on a grid — the start, as wide as the corner's radius; the gap, in the label's own column; the end — so **the gap is sized by layout and not by a measurement**. It is right in the first frame, server-rendered or not, and follows the label when the text or the font changes; the `clip-path` alternative needs the label's width as a number, which only arrives after a `ResizeObserver` has run.

Everything that decides what the pieces look like is in `styles.css` under "The notch": the colour per variant and state, the ring, and whether a `float` label rests. Those are relations between three elements, and three things about them are load-bearing:

- **The notch is the last child of whatever holds the control, and comes after it.** A resting label reads the field's emptiness through `~` — an input's `:placeholder-shown`, a trigger's `data-placeholder` — because both are right before any script runs; `:has()` would be simpler and is past the browser floor. A multiple Combobox has no control that can say it is empty, so it passes `data-empty` itself. An input that may rest a label gets a placeholder of `' '` when it has none, because `:placeholder-shown` never matches an input without one.
- **A Select's notch sits beside its trigger**, in the `NotchFrame`, because the trigger is a `<button>` and a `<label>` cannot go inside one. Everywhere else it is inside the shell, whose hover and focus the edge answers to.
- **`NotchFrame` is what keeps a field as wide as its label.** On the edge the label is absolutely positioned and widens nothing, so a Select of one-word options cut its own name short; the frame lays the label out again, at no height, in the shell's grid cell. A Select and a picker also add the label to their `WidthSizer` samples when it can rest, so a resting label never runs under the chevron.

`labelPlacement` is also the fifth `NebaProvider` default, because where a form's labels go is a decision about the product; each of the ten passes `'labelPlacement'` to `useStyleDefaults`, and the pickers let it ride `...shell` into `PickerShell`.
...
| --- | --- | --- | --- |
| `src/standalone.css` | `dist/styles.css` | `neba/styles.css` | a project with no Tailwind — compiled, ~24 kB gzipped |
...
| all 185 exports               | 291.9 kB | 160.0 kB                    |
...
4. **Fixed-cost modules stay divisible.** A table every component reaches through is a table every component pays for in full. `i18n.ts` is one export per namespace and English only; `icons.tsx` reaches its severity set through a function. The rule generalises: an object literal cannot be tree-shaken per key, so anything that would grow past a few hundred bytes belongs in separate exports or separate modules. **And a class string is written with `+` or a template literal, never as `[…].join(' ')`**, because a bundler folds the first two at build time and drops the constant when nothing reads it, while a _call_ it cannot prove free of side effects stays. Every `join` in `internal/styles.ts` was therefore a fixed cost on the smallest component in the library; the three that were left came to 0.1 kB gzipped on a `Chip`, and two more added in the array form put 0.2 kB on a `Button` before anything read them — which `npm run size` caught as three scenarios over budget at once.
5. **`internal/defaults.ts` stays one `useContext`.** Every component that takes `size`, `density`, `variant`, `locale` or `labelPlacement` calls `useStyleDefaults` before its own destructuring, so whatever is in that module is a fixed cost on the smallest component in the library — it is 0.2 kB gzipped, which is 5% of a Chip. Anything added to it is added to all of them. The hook returns the props object untouched when there is no provider, which is what keeps a page that has none paying only the `useContext`.
...
docs/.vitepress/
  ...
  demos/showcase/app.tsx    # the sample screen on Overview
  theme/
    index.js                # the default theme, with Layout, Demo and PropsTable registered
...
docs/{ko,en}/
  ...
  components/{group}/*.md   # one page per component, grouped (charts, display, feedback, inputs, layout, surfaces, agent, transitions)
...
- **The component groups stay** — they are what say that a Combobox is an input and a Card is a surface, and fifty pages in one list say nothing. Two of them are **placed rather than sorted**: `agent` sits under `surfaces` and `charts` goes last, because each is a set a reader either came for or has no use for, and in the alphabetical run one would open the list and the other would sit in the middle of it. What is flattened is only what is _inside_ a group, so a folder that gains a subfolder does not push its pages a level deeper.
- **Inside a group the pages are sorted by name**, not by their `order` frontmatter. The inputs group alone holds thirty-five components, and nobody remembers where Slider sits in a curated order. `order` still decides inside Design.
...
Adding a component means six edits: a page under `components/{group}/` **in every locale**, its rows in `data/props.ts`, its demo files under `demos/`, a card in `demos/catalog/all.tsx`, a place on the sample screen in `demos/showcase/app.tsx`, and a line in `docs/public/llms.txt`. Only the first is per-locale; the demos and the props data are shared.

The last one is the one that gets forgotten, because nothing renders it and no build fails without it. `llms.txt` is the whole site flattened for an agent, a list of every component the library has, read by something deciding whether to use it at all — so a component missing from it does not exist as far as that reader is concerned. It has no generator; check it against `src/index.ts` when a release is being cut.

[README.md](README.md) deliberately does **not** list the components. It names the groups and links to the docs, which already have every one; a second full list made the README long and was one more place to fall out of step.
...
- **The browser runs in `en-US`**, set once as `contextOptions.locale` in [vitest.config.ts](vitest.config.ts). Without it WebKit takes the host machine's locale, so a number or a date a component formats with the platform default reads differently on a machine set to another language. A test about another language passes a `locale` prop or registers messages; it does not depend on the machine.
...
`npm test` is `scripts/run-tests.mjs` rather than `vitest run`, and what it adds is one thing: the files are split into sessions of at most fifty and each session gets its own browser. Chromium does not survive the whole suite in one — see [Known rough edges](#known-rough-edges) for what that looks like and what was measured. `vitest run` still works and is what a single file should be run with; the runner passes any argument straight through and stops sharding when it is given one, since a filtered run is small by definition and its output is being read.
...
- CI is [.github/workflows/run-test.yml](.github/workflows/run-test.yml), on PRs to `main`, pushes to `main` touching source/test/config paths, and `workflow_dispatch`. Five jobs:
  ...
  - `react-18` — the suite once more with React 18 installed over the lockfile's 19, on ubuntu and chromium only. The peer range promises 18, and this is the only place anything renders on it. A test of something only React 19 has, a function `action`, is skipped there on `React.version` and says why.
...
- **A chromium run loses its browser partway through the suite, and it is not ours.** It reads either as `Failed to run the test <file>` / `[vitest] Browser connection was closed while running tests` or, on Windows, as a file that prints `(0 test)` and then never says anything again. **Read the counts before chasing the component.** The run exits 1 on that one unhandled error, every test that got to start passed, and the file count comes up short only because the files still queued behind the dead browser never run at all — `Test Files 113 passed (146)` over `Tests 2389 passed (2389)` is this and not a regression.
  ...
  It is chromium-only — firefox and webkit have never dropped a file — and it has been seen on all three runners. It has landed on a dozen different files, so no single test is guilty; what they share is a **position**, somewhere past the hundredth file. Vitest's own tracker has the report open against browser mode.
