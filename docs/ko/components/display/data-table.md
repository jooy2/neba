...
고정은 열을 **옮기기도** 합니다. `columnOrder`가 뭐라 했든 start로 고정된 것이 먼저, end로 고정된 것이 마지막에 그려집니다.

고정한 열에는 `width`를 주세요. 너비가 없으면 sticky 셀이 앉는 offset을 그 열의 실제 너비가 아니라 기본 너비로 계산합니다.
...
`reorderable`은 헤더를 행을 따라 끌 수 있게 합니다. 기본값은 꺼짐이며, 드래그는 누르는 순간이 아니라 일정 거리를 움직인 뒤에 시작하므로 정렬하려던 클릭이 열을 옮기지 않습니다. 고정된 헤더는 끌 수 없습니다.
...
어느 하나만으로는 동작하지 않습니다. 위에 핸들러가 없는 열은 `editable`을 어떻게 두든 편집되지 않습니다. 표는 **행의 사본을 갖지 않습니다**. 새 값을 넘기고, `items`로 돌아온 것을 그립니다.

`editable`은 함수일 수 있습니다. 잠긴 레코드나 계산된 필드를 위해서입니다. `editType: 'number'`는 휴대폰에서 숫자 키패드를 유지하고 문자열이 아니라 숫자를 돌려줍니다.

더블 클릭이나 활성 행에서 <kbd>F2</kbd>를 누르면 에디터가 열리고, blur와 `Enter`가 확정하고 `Escape`가 취소합니다. 두 키 모두 focus를 표로 돌려줍니다. 에디터를 연 더블 클릭에는 `onRowActivate`가 **함께 발동하지 않습니다**.
...
그룹화는 검색과 정렬 **다음에** 일어납니다. 정렬된 표는 각 그룹 안에서 정렬을 유지하고, 걸러진 표는 남은 것만 묶습니다. 그룹은 첫 행이 나타난 순서를 지킵니다. `groupBy`가 `undefined`를 돌려준 행들만 예외로 맨 위에 가며, 이 행들의 제목은 표의 `locale`로 쓴 "그룹 없음"입니다.

열의 `aggregate`는 한 그룹의 행들을 받아 호출되고, 돌려준 값은 그룹 제목 줄의 그 열에 그려집니다. 늘 함수이며 `'sum' | 'avg'` 같은 축약형은 없습니다.
...
파일에는 지금 보고 있는 페이지만이 아니라 지금 보고 있는 모든 행이 들어갑니다. 검색과 정렬은 적용되고 페이징은 적용되지 않습니다.

열의 `exportValue`가 파일에 들어가는 값입니다. 없으면 `value`를, 그것도 없으면 `row[key]`를 쓰며, `render`가 그린 것은 쓰지 않습니다. 열의 `exportable: false`는 그 열을 빼냅니다.

파일은 byte-order mark로 시작하므로, Excel에서도 ASCII가 아닌 글자가 깨지지 않습니다. `=`, `+`, `-`, `@`, 탭, 캐리지 리턴으로 시작하는 텍스트 셀은 앞에 `'`를 붙여 쓰므로, 스프레드시트가 수식으로 실행하지 않고 글자로 보여 줍니다. `exportEscapeFormulas={false}`면 그대로 씁니다.
