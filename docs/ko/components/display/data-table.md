...
고정한 열에는 `width`를 주세요. sticky 셀이 앉는 offset은 그 앞 열들의 너비 합이고, 너비를 말하지 않은 열은 더할 숫자가 없어 기본값으로 재게 됩니다. 그건 추측입니다.
...
`reorderable`은 헤더를 행을 따라 끌 수 있게 합니다. 기본값은 꺼짐입니다 — 헤더는 컨트롤이고, 정렬하려던 사용자의 손짓에 열이 움직이는 표는 열이 아예 안 움직이는 표보다 나쁩니다. 드래그가 누름이 아니라 임계값에서 시작하는 것도 같은 이유입니다. 고정된 헤더는 끌 수 없습니다. 그 자리는 고정이 정한 것이니까요.
...
`aggregate`는 그룹 제목 줄의 **자기 열에** 그려집니다. 그게 요점입니다 — 그룹의 합계는 그것이 합계인 숫자들과 같은 열에 있어야 합니다. `'sum' | 'avg'` 같은 축약형은 없습니다. 가중 평균이나 고유 개수가 필요한 열이 하나 생기는 순간, 절반은 함수이고 절반은 문자열이 됩니다.
...
파일은 byte-order mark로 시작하고, 이건 장식이 아닙니다. Excel은 BOM 없는 UTF-8 CSV를 로컬 코드 페이지로 읽어서 ASCII가 아닌 이름이 전부 깨져 도착합니다.
