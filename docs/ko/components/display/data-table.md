...
`reorderable`은 헤더를 행을 따라 끌 수 있게 합니다. 기본값은 꺼짐입니다 — 헤더는 컨트롤이고, 정렬하려던 사용자의 손짓에 열이 움직이는 표는 열이 아예 안 움직이는 표보다 나쁩니다. 드래그가 누름이 아니라 임계값에서 시작하는 것도 같은 이유입니다. 고정된 헤더는 끌 수 없습니다. 그 자리는 고정이 정하기 때문입니다.
