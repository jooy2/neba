...
산점도는 어떤 두 마크든 나란히 놓일 수 있는 형태이므로, 팔레트가 이웃한 쌍이 아니라 **모든 쌍**에서 구분되어야 합니다. 그렇게 검사하면 구분되는 것은 세 series까지입니다. 넷째부터는 `shape="auto"`가 series마다 다른 모양을 — circle, square, triangle, diamond, cross 순서로 — 내어 줍니다. 범례에도 같은 모양이 나옵니다.
