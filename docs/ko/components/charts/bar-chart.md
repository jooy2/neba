...
막대의 **길이**가 곧 값이므로, 값 축은 기본으로 0을 포함합니다. `yAxis.min`과 `yAxis.max`로 양 끝을 고정할 수는 있지만, 축을 자르면 두 배 긴 막대가 두 배를 뜻하지 않게 됩니다. category에 자연스러운 순서가 있고 변화의 모양이 핵심이라면 [LineChart](./line-chart)가 더 나은 마크입니다.
