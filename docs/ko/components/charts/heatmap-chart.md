...
`min`·`max`는 스케일을 고정합니다. 생략하면 양 끝이 데이터에서 오는데, 그 말은 서로 다른 데이터의 두 차트는 같은 범위를 줄 때까지 비교할 수 없다는 뜻입니다. sequential 스케일에서 값이 모두 같으면 나눌 범위가 없으므로, 0 이하는 가장 연한 단계, 0보다 크면 가장 진한 단계가 됩니다.
