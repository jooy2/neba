...
지정하지 않으면 상자는 담고 있는 것의 높이 그대로입니다. 문단이나 사진에는 그쪽이 맞습니다. `maxHeight`는 덮여 있는 동안의 높이를 제한하고, 열면 그 제한이 풀려 내용이 필요한 높이를 그대로 차지합니다. CSS 길이 또는 픽셀 수를 받습니다.

`reversible`은 열고 난 뒤 다시 덮을 수 있게 내용 아래에 닫기 버튼을 둡니다. 그 줄은 아직 덮여 있는 동안에도 자리를 지키므로 상자 높이는 두 상태에서 같고, 버튼을 눌러도 페이지가 밀리지 않습니다.
