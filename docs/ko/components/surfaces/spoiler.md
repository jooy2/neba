...
`maxHeight`를 지정하지 않으면 상자는 담고 있는 것의 높이 그대로입니다. 덮개는 내용이 드러난 뒤에도 자리를 지키므로, 누르는 순간 상자가 줄어들지 않고 아래 내용도 밀리지 않습니다.

`maxHeight`는 덮여 있는 동안의 높이를 CSS 길이나 픽셀 수로 제한합니다. 열면 그 제한이 풀려 내용이 필요한 높이를 그대로 차지합니다.

`reversible`은 내용 아래에 닫기 버튼을 두어 다시 덮을 수 있게 합니다. 그 줄은 아직 덮여 있는 동안에도 자리를 지키므로, 다시 덮어도 상자 높이가 바뀌지 않습니다.
...
- 덮여 있는 동안 내용은 `inert`입니다. tab 순서에서 빠지고, 접근성 트리에서 사라지며, 전체 선택에도 걸리지 않습니다.
