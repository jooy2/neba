...
각 단계는 glyph를 하나 받을 수 있고, 그 단계 본문의 제목 앞에 그려집니다. 목록에는 그리지 않습니다. 목록의 행에는 이미 번호가 붙은 원이 있어서, 그 옆의 glyph는 같은 말을 두 번 하게 되기 때문입니다. 아이콘은 이것이 터미널 작업인지 파일 작업인지 경고인지처럼 _어떤 종류의_ 단계인지를 나타낼 때 잘 맞습니다.
...
- 한 페이지에 안내서가 둘 이상이면 `title`을 주십시오. `title`이 있으면 안내서는 그 제목으로 이름이 붙은 `role="group"`이 됩니다.
