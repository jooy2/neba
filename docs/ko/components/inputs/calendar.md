...
스케줄러가 아닙니다. 칸의 크기는 컨트롤 사다리의 높이이고(`md`에서 32px), 그래서 `renderDay`는 숫자 아래에 점 하나, 개수 하나, 막대 하나를 놓을 자리이지 하루치 일정을 담을 자리가 아닙니다. 그것을 그리는 컴포넌트는 다른 그리드가 필요하고, 이것을 그렇게 부르는 것은 크기가 지킬 수 없는 약속입니다.
...
| 키                    | 동작                                       |
| --------------------- | ------------------------------------------ |
| `←` `→` `↑` `↓`       | 하루 또는 한 주 이동, 가장자리에서 달 넘김. RTL에서는 `←`가 다음 날 |
| `Home` / `End`        | 주의 처음 또는 끝                          |
