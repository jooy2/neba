...
- `disabled`와 `readOnly`는 둘 다 패널과 레일, 스와치를 tab 순서에서 빼고 포인터와 키보드에 반응하지 않게 합니다. `disabled`는 텍스트 필드도 tab 순서에서 빼고, `readOnly`에서는 텍스트 필드가 읽기 전용으로 tab 순서에 남아 값을 선택하고 복사할 수 있습니다.
