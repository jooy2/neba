...
- 모든 목적지가 `<a href>`이므로 새 탭에서 열고, 주소를 복사하고, 크롤러가 따라가고, 스크린 리더의 링크 목록에서 도달할 수 있습니다. 패널의 링크는 패널을 처음 열기 전에도 숨긴 채 렌더되므로 서버 렌더에도 들어갑니다. 항목에 `keepMounted={false}`를 주면 열 때까지 빠집니다.
