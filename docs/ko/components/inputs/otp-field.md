...
나머지 `<div>` 속성은 그대로 칸들의 행에 전달됩니다. `color`·`defaultValue`·`onChange`·`children`은 `<div>`의 것 대신 필드 자신의 것을 씁니다. 값은 `onValueChange`가 알려 주고, 칸은 필드가 `length`에 맞춰 직접 그립니다. 공용 축은 [prop 규약](../../design/prop-conventions)에 있습니다.
