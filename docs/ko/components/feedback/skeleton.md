...
- 한 영역 전체를 대표하는 **하나**에만 `label`을 주면 그 label을 화면에 보이지 않는 글자로 담은 `status`가 됩니다. 그 글자를 담은 채 마운트되므로 읽는 사람이 그 자리에 오면 읽히지만, 나타나는 순간에 모든 screen reader가 알려 주지는 않습니다. 쌓인 막대마다 label을 붙이지 마세요.
