...
모든 네이티브 `<span>` 속성이 그대로 전달됩니다. 자체 표면을 그리지 않으므로 `variant`, `color`, `elevation`은 없습니다.

상자 크기가 더미와 정확히 일치하므로 Stack 뒤에 오는 내용도 올바른 너비를 기준으로 배치됩니다. `size`는 `overlap`의 기본값을 정할 때에만 읽습니다.

자식은 손대지 않은 채 통과하므로 무엇이든 쌓을 수 있습니다. 아바타를 감싼 [Tooltip](../feedback/tooltip)도, 다른 컴포넌트의 `.map()`이 만들어 낸 것도 됩니다. `ring`은 자식 자체에 그려지므로 그 모양을 따라갑니다.
...
- Stack은 `<span>`이고 아무것도 알리지 않습니다. 더미가 한 무리를 뜻한다면(작업에 붙은 사람들, 폴더 안의 파일들) `role`과 이름을 주거나 개수를 옆에 글로 적으세요.
