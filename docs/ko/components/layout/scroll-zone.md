...
기본값 `auto`는 갈 곳이 있는 버튼만 그리고, 전부 들어맞을 때에는 둘 다 그리지 않습니다. `always`는 첫 페인트부터 둘 다 그리고 갈 곳이 없는 쪽을 disabled로 둡니다 — 내용이 나중에 도착하는 스트립이 원하는 쪽입니다. `none`은 아무것도 그리지 않고 드래그와 방향키, 포인터가 이미 밀 수 있는 것에 맡깁니다.
