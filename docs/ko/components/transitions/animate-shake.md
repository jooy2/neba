...
컨트롤을 transform하지 않는다는 것이 이 라이브러리의 규칙입니다. 이 규칙은 hover, press, on, off 같은 컨트롤의 **정지 상태**에 적용되며, 그런 상태는 색으로 나타내는 편이 더 분명합니다. 흔들림은 상태가 아닙니다. 독자가 방금 한 조작에 대한 일회성 응답이고 400ms 만에 끝나며, 실패했다는 것을 이만큼 분명하게 전달하는 색은 없습니다.
