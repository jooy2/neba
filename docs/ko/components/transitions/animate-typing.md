...
- 캐럿은 줄을 다 친 뒤에도 페이지에 있는 동안 계속 깜박이고, `paused`로도 멈추지 않습니다. 독자가 읽어 나가는 글 옆에 둘 때는 `caret={false}`를 주세요.
- 상자의 크기는 첫 프레임부터 문자열 전체로 정해지므로, 줄이 쳐지는 동안 주변 텍스트가 다시 배치되지 않습니다.
