...
- 축소된 모션 설정에서는 릴이 스스로 돌지 않고 보이던 줄에 머뭅니다. controlled `index`를 바꾸면 줄이 바뀌며, 미끄러지는 대신 그냥 나타납니다.
- 그 밖의 경우에는 5초 넘게 계속 바뀌므로 페이지에 멈출 방법이 있어야 합니다. 옆에 일시정지 버튼을 두고 그 상태를 `paused`에 연결하세요.
