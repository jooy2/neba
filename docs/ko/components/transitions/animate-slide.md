...
어느 변에서 들어오는지 정하며, `top`, `right`, `bottom`, `left`를 받습니다. 라이브러리의 다른 곳에서 `NebaSide`가 그렇듯 논리적이 아니라 물리적입니다. 위에서 내려오는 패널은 어떤 쓰기 방향에서도 위에서 내려옵니다.
...
- 축소된 모션 설정에서는 애니메이션이 마지막 프레임으로 바로 넘어가고, 내용은 제자리에 그냥 있거나 나가는 중이었다면 사라집니다.
