...
다만 전역에 적용되므로, 이미 자체 마크업이 있는 페이지에 스타일시트를 넣으면 브라우저 기본값에 기대던 아래 항목이 바뀝니다.

- 모든 요소가 `border-box`가 됩니다.
- 제목, 문단, `blockquote`, `figure`, `pre`, `hr`의 margin이 없어지고, 제목은 주변 글자와 같은 크기와 굵기가 됩니다.
- `ul`, `ol`, `menu`의 마커와 들여쓰기가 없어집니다. Safari의 VoiceOver는 CSS로 마커를 없앤 목록을 목록으로 읽지 않으므로, 본문 속 목록이 목록으로 읽혀야 하면 `role="list"`를 붙이세요.
- 직접 쓴 `<button>`, `<input>`, `<select>`, `<textarea>`의 테두리와 배경이 없어지고 모서리가 각지게 됩니다.

되살리고 싶은 항목은 직접 CSS로 스타일을 주세요. 이미 Tailwind를 쓰는 프로젝트라면 reset이 없는 아래 경로를 쓰면 됩니다.
