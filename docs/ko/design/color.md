...
`on-tint`는 그 짝의 세 번째이고, 고르는 값이 아니라 파생값입니다. `panel`과 `soft`는 둘 다 `accent`를 옅게 푼 워시라, 그 위에 `accent`로 글자를 쓰면 자기 자신의 옅은 사본 위에 자기 색을 얹는 꼴이 됩니다. 글자와 바닥이 함께 움직이니 어느 한쪽을 조정해서는 둘 사이를 벌릴 수 없습니다. 글자를 `--neba-fg` 쪽으로 28% 당기면 그 연동이 끊어지고, 테마에 따라 방향도 알아서 뒤집힙니다.
...
```css
--neba-panel        /* 흰색 66% (다크 5%) */
--neba-panel-hover  /* 흰색 82% (다크 7%) */
--neba-panel-press  /* 흰색 92% (다크 9%) */
```
...
이 사다리를 뒤집는 표면이 딱 하나 있습니다. Switch의 thumb가 달리는 트랙입니다.

```css
--neba-groove        /* 검정 14% (다크: 흰색 5%) */
--neba-groove-hover  /* 검정 19% (다크: 흰색 7%) */
```

panel은 페이지 위에 흰색을 얹는 값이라, 라이트 테마에서는 꺼진 트랙이 그것이 얹힌 바탕보다 밝아졌습니다. thumb는 `--neba-surface`, 곧 흰색입니다. 흰 원반이 흰 트랙 위에 있으면 thumb가 아니라 빈 알약입니다. 홈은 반대로 뒤를 어둡게 눌러 주고, 그래야 thumb가 얹힐 빛이 됩니다. 계열 색이 들어가지 않는 것은 켜졌다는 말을 하는 쪽이 계열이기 때문입니다.
...
- **`accent`는 흰 배경에서 5:1 이상**입니다. `accent`가 쓰이라고 만들어진 바닥이 그쪽입니다. 자기 계열 색으로 물든 바닥 위에서는 `on-tint`를 쓰고, 이쪽은 `panel`과 `soft` 두 사다리의 모든 단계에서 두 테마 모두 4.5:1을 넘깁니다.
- **채도는 sRGB gamut 상한의 90% 근처**입니다. 상한을 한참 밑돌면 같은 밝기에서도 회색에 가깝게 읽히고, 넘기면 브라우저가 clip합니다.
- **다크에서 글자색은 맨 면이 아니라 띄운 면 위에서 잽니다.** panel 사다리는 불투명도라서 흰 페이지에서는 비용이 없고 검정에 가까운 페이지에서는 큽니다. 이유는 [디자인 언어](./design-language)에 있습니다.
