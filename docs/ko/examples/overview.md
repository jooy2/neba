...
<p class="neba-lede">라이브러리의 컴포넌트 대부분을 하나의 화면에 배치한 샘플입니다. 컴포넌트를 낱개로 늘어놓는 대신 실제 제품 화면처럼 구성해서, 함께 놓였을 때의 크기와 정렬을 확인할 수 있게 했습니다.</p>
...
| 배포 | `Tabs` `Table` `Badge` `Pagination` | 표는 열 목록으로 그려지므로 머리글과 셀이 서로 어긋날 수 없습니다 |
