...
describe('TextField', () => {
  ...
  describe('style props', () => {
    it('maps color onto the token slots the styles read from', async () => {
      ...
      expect(root.style.getPropertyValue('--n-accent')).toBe('var(--neba-success-accent)');
    ...
    it('defaults to the primary color', async () => {
      ...
      expect(root.style.getPropertyValue('--n-accent')).toBe('var(--neba-primary-accent)');
    });
    ...
    it('changes horizontal padding with density', async () => {
      ...
      expect(shell).not.toHaveClass('px-5');
    });
    ...
    it('is flat by default and maps elevation onto the shadow scale', async () => {
      ...
      expect(root.style.getPropertyValue('--n-elev')).toBe('var(--neba-shadow-0)');
    ...
    it('draws a border for the outline variant only', async () => {
      const screen = await render(<TextField variant="outline" />);
      const shell = screen.getByRole('textbox').element().parentElement;

      expect(shell).toHaveClass('border');

      await screen.rerender(<TextField variant="text" />);
  ...
  describe('states', () => {
    it('disables the control and drops the color family', async () => {
      const screen = await render(<TextField label="Email" disabled />);
      const input = screen.getByRole('textbox').element();

      expect(input).toBeDisabled();
      expect(input.parentElement).toHaveClass('text-(--neba-disabled-fg)');
    });
    ...
    it('keeps its color but goes flat and desaturated when read-only', async () => {
      ...
      expect(shell).toHaveClass('[box-shadow:var(--n-elev),var(--neba-plate-glass)]');
      expect(shell).not.toHaveClass('[filter:saturate(0.55)]');

      await screen.rerender(<TextField elevation={2} readOnly />);

      expect(shell).toHaveClass('[box-shadow:var(--neba-plate-glass)]');
      expect(shell).not.toHaveClass('[box-shadow:var(--n-elev),var(--neba-plate-glass)]');
      expect(shell).toHaveClass('[filter:saturate(0.55)]');
    });
  ...
  describe('validity', () => {
    it('turns the whole slot family over to danger when invalid', async () => {
      ...
      expect(root.style.getPropertyValue('--n-ring')).toBe('var(--neba-primary-ring)');
