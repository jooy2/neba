 ...
 * too small to read a code out of across a desk. Every step is taller than it is
 * wide, which is what makes a row of them read as places for one character each
 * rather than as a row of tiny fields.
 */
const slotSizeClasses: Record<NebaSize, string> = {
  xs: 'h-7 w-6',
  sm: 'h-8 w-7',
  md: 'h-10 w-8',
  lg: 'h-12 w-10',
 ...
 *
 * `radiusClasses` is ~45% of a *control's height*, which is a cut corner on
 * something wide and a lozenge on something nearly square: `md` is 14px, and a
 * 14px corner on a 32px-wide box is a pill. These are ~22% of the slot's width
 * instead, which is the same amount of cut on this shape — a sheet with the
 * corners taken off, never a pill.
 */
const slotRadiusClasses: Record<NebaSize, string> = {
 ...
 */
const slotTextClasses: Record<NebaSize, string> = {
  xs: 'text-[0.8125rem]',
  sm: 'text-[0.9375rem]',
  md: 'text-[1.0625rem]',
  lg: 'text-[1.25rem]',
 ...
 */
const slotGapClasses: Record<NebaDensity, Record<NebaSize, string>> = {
...
const validationTypes: Record<OtpFieldCharset, 'numeric' | 'alpha' | 'alphanumeric' | 'none'> = {
  numeric: 'numeric',
  alpha: 'alpha',
