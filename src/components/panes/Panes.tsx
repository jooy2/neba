 ...
 */
const handleTrackClasses: Record<NebaSize, string> = {
  xs: 'basis-1',
  sm: 'basis-1.5',
  md: 'basis-2',
  lg: 'basis-2.5',
...
const handleTrackValues: Record<NebaSize, number> = {
  xs: 4,
  sm: 6,
  md: 8,
  lg: 10,
...
export const Panes = React.forwardRef<HTMLDivElement, PanesProps>(function Panes(
  ...
  return (
    <div
      ...
      // Three slots rather than the whole `surfaceSlots` set: a Panes draws no
            ...
            >
              ...
              */}
              <span
                aria-hidden="true"
                className={cx(
                  'pointer-events-none bg-(--neba-border)',
                  transitionClasses,
                  horizontal ? 'h-full w-px' : 'h-px w-full',
                  resizable
                    ? 'group-hover/handle:bg-(--n-accent) group-focus-visible/handle:bg-(--n-accent) group-data-[dragging]/handle:bg-(--n-accent)'
                    : ''
                )}
              />
            </div>
