 ...
 * position.
 */
const buttonInsetClasses: Record<NebaSize, string> = {
  xs: 'p-1',
  sm: 'p-1.5',
  md: 'p-2',
  lg: 'p-3',
 ...
 */
export const ScrollZone = React.forwardRef<HTMLDivElement, ScrollZoneProps>(function ScrollZone(
  {
  ...
  const measure = React.useCallback(() => {
    ...
    setReach((previous) => {
      ...
      return previous.back === back && previous.forward === forward ? previous : { back, forward };
    });
  }, [horizontal]);
  ...
  const advance = React.useCallback(
    (forward: boolean, unit: ScrollZoneMode = mode) => {
      const el = scrollerRef.current;
      if (!el) return;
      ...
      // ScrollZone are whatever the caller put there, so no two of them are
   ...
   * torn down through one ref, because the pointerup that would normally end it
   * never arrives if the zone is unmounted mid-press.
   */
  ...
  const beginHold = React.useCallback(
    (forward: boolean, stop: 'pointerup' | 'keyup') => {
      ...
      const release = () => {
        ...
        if (performance.now() - started < TAP_MS) advance(forward, 'item');
      };
  ...
  function beginDrag(event: React.PointerEvent<HTMLDivElement>) {
    if (!drag || event.pointerType === 'touch' || event.button !== 0) return;
    ...
    const move = (moveEvent: PointerEvent) => {
      ...
      if (!dragging) {
        if (Math.abs(horizontal ? dx : dy) < DRAG_THRESHOLD) return;
        dragging = true;
  ...
  function pressHandlers(forward: boolean) {
    ...
    return {
      ...
      // The keyboard's own half of a hold. Without it the button is a control a
      // pointer can use and a keyboard cannot, which is the one thing a scroll
      // affordance must never be.
    ...
    >
      ...
      >
        <div
          ...
          // The whole track is laid out inline rather than in utilities, for the
          // reason `internal/grid.ts` gives: `repeat(3, auto)` is a count the
          // caller picked and Tailwind only ever sees class names written out
          // literally. Half of the declaration in a class and half of it here
          // would be two places to read one layout.
          //
          // A grid rather than a flex row, because `lines` is exactly the thing
          // flex cannot state: a wrapping row wraps at the viewport, and what is
          // wanted here is a fixed number of rows and as many columns as it
          // takes.
          style={{
            display: 'grid',
