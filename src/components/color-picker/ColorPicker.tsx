 ...
 * read, and one 32px tall would give the pointer four hundred distinguishable
 * colours out of a possible ten thousand.
 */
const panelWidthClasses: Record<NebaSize, string> = {
  xs: 'w-40',
  sm: 'w-44',
  md: 'w-52',
  lg: 'w-60',
...
const areaHeightClasses: Record<NebaSize, string> = {
  xs: 'h-24',
  sm: 'h-28',
  md: 'h-32',
  lg: 'h-40',
...
const railHeightClasses: Record<NebaSize, string> = {
  xs: 'h-2.5',
  sm: 'h-3',
  md: 'h-3.5',
  lg: 'h-4',
...
const panelGapClasses: Record<NebaSize, string> = {
  xs: 'gap-1.5',
  sm: 'gap-2',
  md: 'gap-2.5',
  lg: 'gap-3',
 ...
 */
const thumbSizes: Record<NebaSize, number> = { xs: 10, sm: 12, md: 14, lg: 16, xl: 18 };
...
interface PanelProps {
   ...
   */
  onChange: (next: { hsv: Hsv; alpha: number }) => void;
  text: string;
    ...
    <div className={cx('flex flex-col', panelWidthClasses[size], panelGapClasses[size])}>
      ...
      {editable ? (
        <div className={cx('flex items-center', panelGapClasses[size])}>
          <span
            aria-hidden="true"
            className={cx('shrink-0 rounded-full border', '[border-color:var(--neba-border)]')}
...
function widthSamples(format: ColorFormat, withAlpha: boolean): string[] {
  if (format === 'hex') {
...
const fallbackHsv: Hsv = { h: 217, s: 87, v: 82 };
