 ...
 */
const widthValues: Record<NebaSize, string> = {
  xs: '11rem',
  sm: '13rem',
  md: '16rem',
  lg: '18rem',
  ...
  return (
    <aside
      ...
      className={cx(
        ...
        divider
          ? cx(side === 'start' ? 'border-e' : 'border-s', '[border-color:var(--n-line)]')
          : '',
        layout.scroll === 'content'
          ...
          : sticky
            ? cx(
                'self-start',
                'sticky [top:var(--n-layout-header,0px)]',
                '[height:calc(100dvh-var(--n-layout-header,0px)-var(--n-layout-footer,0px))]'
              )
            : '',
        // What holds the first paint together. The server has no window to
        // measure, so it sends the column; this is what keeps that column off a
        // screen too narrow for it until `matchMedia` can say the same thing.
      ...
      {resizable ? (
        <div
          ...
          className={cx(
            ...
            'neba-hit absolute inset-y-0 z-1 w-2 cursor-col-resize touch-none',
