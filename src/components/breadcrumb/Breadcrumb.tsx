...
const trailGapClasses: Record<NebaDensity, Record<NebaSize, string>> = {
  default: { xs: 'gap-1', sm: 'gap-1.5', md: 'gap-2', lg: 'gap-2.5', xl: 'gap-3' },
  compact: { xs: 'gap-0.5', sm: 'gap-1', md: 'gap-1', lg: 'gap-1.5', xl: 'gap-2' }
};

/**
 * A step's corner, well below the control ladder.
 *
 * `radiusClasses.md` is 14px, which on a line of text 20px tall is a pill — and
 * a breadcrumb step is not a Chip. What the hover tint needs is a rectangle with
 * the corners taken off, which is what the ladder says everywhere else; it just
 * has to be read two steps down for something this short.
 */
const stepRadiusClasses: Record<NebaSize, string> = {
  xs: radiusClasses.xs,
  sm: radiusClasses.xs,
  md: radiusClasses.xs,
  lg: radiusClasses.sm,
 ...
 * the no-transform rule makes for a chevron on an Accordion: the wedge is drawn
 * pointing down once, and a trail turns it a quarter. Both turn back under RTL,
 * because a trail runs the way the language does.
 */
function separatorMark(separator: BreadcrumbSeparator): React.ReactNode {
  switch (separator) {
    case 'chevron':
      return (
