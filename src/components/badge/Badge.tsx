 ...
 */
const badgeHeightClasses: Record<NebaSize, string> = {
  xs: 'h-3.5 min-w-3.5',
  sm: 'h-4 min-w-4',
  md: 'h-4.5 min-w-4.5',
  lg: 'h-5 min-w-5',
...
const dotSizeClasses: Record<NebaSize, string> = {
  xs: 'size-1.5',
  sm: 'size-2',
  md: 'size-2.5',
  lg: 'size-2.5',
...
const badgeTextClasses: Record<NebaSize, string> = {
  xs: 'text-[0.5625rem]',
  sm: 'text-[0.625rem]',
  md: 'text-[0.6875rem]',
  lg: 'text-[0.75rem]',
...
const badgePaddingClasses: Record<
  NonNullable<NebaStyleProps['density']>,
  Record<NebaSize, string>
> = {
...
/**
 * A badge sits well below the radius ladder the sheets use — at 18px tall,
 * `--neba-radius-xs` (10px) is already past the halfway point that would make it
 * a pill, and a badge *is* the one thing in the library allowed to be one.
 *
 * That is not a hole in the design language, it is the exception the language
 * names: the flat run along a sheet's edge is what says "this is a cut surface".
 * A badge is not a surface. It is a mark laid on one, and a mark has no edge to
 * cut — which is also why it is the only component that overlaps its neighbour.
 */
 ...
 */
const cornerOffsets: Record<NebaSize, { badge: string; dot: string }> = {
 ...
 */
const placementClasses: Record<NebaCorner, string> = {
 ...
 */
const circleInsetClasses: Record<NebaCorner, string> = {
  'top-start': 'mt-[7%] ms-[7%]',
  'top-end': 'mt-[7%] me-[7%]',
...
/**
 * The three weights, said the way a *control* says them: the marker is the thing
 * being coloured, so its panel takes the tint.
 *
 * `text` is the one to reach for on a busy surface — a soft tinted mark that
...
const variantClasses: Record<NonNullable<NebaStyleProps['variant']>, string> = {
  ...
  text: `text-(--n-on-tint) ${tintPlateClasses}`
};
