...
export function revealClip(side: NebaSide): string {
  switch (side) {
    case 'top':
...
export function animationSlots(options: AnimationSlotOptions): React.CSSProperties {
  ...
  if (options.clip !== undefined) {
    slots['--n-anim-clip'] = options.clip;
  }

  // Only written when it is asked for. The default in `styles.css` is `auto`,
  // which is the clock, and a slot filled with it on every animated element in
  // the library would be the same answer written a hundred and thirty times.
  if (options.timeline === 'view') {
    slots['--n-anim-timeline'] = 'view()';
    slots['--n-anim-range'] = options.range ?? 'entry 0% cover 45%';
  }
...
/* ---------------------------------------------------------------------------
 * Handing an effect to the children
 * ------------------------------------------------------------------------- */
...
export function staggers({ stagger, durationStep }: StaggerOptions): boolean {
  return stagger !== 0 || durationStep !== 0;
}
...
export function staggerChildren(
  ...
  { stagger, durationStep, reverse }: StaggerOptions
): React.ReactNode[] {
  ...
  return items.map((child, index) => {
    ...
    const style = animationSlots({
      ...slots,
      delay: slots.delay + order * stagger,
      // A run that has been sped up past nothing is not a run backwards; it is
      // a cut. Clamped, so a `durationStep` steeper than the list is long stops
      // at instant rather than turning the tail into `-200ms`.
      duration: Math.max(0, slots.duration + order * durationStep)
    });

    if (!React.isValidElement(child)) {
      return (
        <span key={index} className={effectClass} style={style}>
          {child}
        </span>
      );
    }
    ...
    return React.cloneElement(child as React.ReactElement<Record<string, unknown>>, {
      className: cx(effectClass, childProps.className),
...
export function transitionParts(
  ...
  if (type === 'reveal') {
    slots.clip = revealClip(options.side ?? 'left');
  }

  return { effect: type, slots };
}
...
export function useAnimateElement(params: AnimateElementParams): AnimateElement {
  ...
  const state = slots.timeline === 'view' ? 'running' : run.state;

  return {
    ref: run.ref,
    // Nothing on the root once the children are the ones animating, or the box
    // would fade in over the top of eight children fading in.
    className: spread ? '' : effectClass,
    style: (spread
      ? { '--n-anim-state': state }
      : { ...animationSlots(slots), '--n-anim-state': state }) as React.CSSProperties,
    props: {
      ...run.handlers,
