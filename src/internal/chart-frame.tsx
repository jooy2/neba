...
/**
 * The strip under the plot, loaded rather than imported.
 *
 * The frame is in every cartesian chart's bundle, so a component sitting in it
 * is a cost every chart pays whether it draws one or not — and most charts
 * have nothing long enough to need a window. Behind a chunk of its own it
 * costs a chart that never asks nothing at all, and a chart that does asks for
 * it while the plot, which narrowed synchronously, is already drawn.
 */
const ChartBrush = React.lazy(() =>
  import('./chart-brush.js').then((module) => ({ default: module.ChartBrush }))
);
