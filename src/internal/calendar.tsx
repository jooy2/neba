 ...
 *
 * It goes in an inline `--n-cell` slot for the reason the design language gives
 ...
 *
 * The numbers are `controlHeightClasses` as lengths: a `md` day cell is 32px,
 * which is a `md` Button, which is a `md` TextField. A calendar dropped beside a
 * form is on the form's grid.
 */
const cellSizes: Record<NebaSize, string> = {
  xs: '1.375rem',
  sm: '1.625rem',
  md: '2rem',
  lg: '2.5rem',
...
const cellRadiusClasses: Record<NebaSize, string> = {
  xs: 'rounded-(--neba-radius-xs)',
  sm: 'rounded-(--neba-radius-xs)',
  md: 'rounded-(--neba-radius-sm)',
  lg: 'rounded-(--neba-radius-md)',
  xl: 'rounded-(--neba-radius-lg)'
};

/** The same corner on one side only, for where a range band stops. */
const cellRadiusStartClasses: Record<NebaSize, string> = {
  xs: 'rounded-s-(--neba-radius-xs)',
  sm: 'rounded-s-(--neba-radius-xs)',
  md: 'rounded-s-(--neba-radius-sm)',
  lg: 'rounded-s-(--neba-radius-md)',
  xl: 'rounded-s-(--neba-radius-lg)'
};

const cellRadiusEndClasses: Record<NebaSize, string> = {
  xs: 'rounded-e-(--neba-radius-xs)',
  sm: 'rounded-e-(--neba-radius-xs)',
  md: 'rounded-e-(--neba-radius-sm)',
  lg: 'rounded-e-(--neba-radius-md)',
  xl: 'rounded-e-(--neba-radius-lg)'
};

/**
 * What every cell in every one of the three views is drawn on.
 *
 * The house signature, unchanged: per-property durations, `:active` zeroing them
 * so a press lands on the frame of the click, and no `transform` anywhere near
 * it. A grid of forty-two cells that each grew a pixel under the pointer would
 * be the single worst place in the library to break that rule.
 *
...
function Header({
  ...
  const stepper = (direction: -1 | 1, shown: boolean) =>
    shown ? (
      ...
      <span aria-hidden="true" className={cx(controlHeightClasses[size], 'w-(--n-cell)')} />
    );
    ...
    <div className="flex items-center justify-between gap-1">
      ...
      <div className={cx('flex min-w-0 flex-1 items-center justify-center', gapClasses[size])}>
        {view === 'year' ? (
          ...
          <span
            className={cx(
              'flex items-center font-medium tabular-nums text-(--neba-fg)',
...
export function Calendar({
  ...
  return (
    <div
      ...
      style={{ '--n-cell': cellSizes[size] } as React.CSSProperties}
      ...
      <div role="row" className="grid grid-cols-7">
        {short.map((label, index) => (
          <span
            ...
            className={cx(
              'flex h-(--n-cell) items-center justify-center font-medium text-(--neba-muted-fg) select-none',
