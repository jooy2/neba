...
const systems: Record<NebaMockupDevice, readonly NebaMockupOs[]> = {
 ...
 */
export function resolveOs(device: NebaMockupDevice, os?: NebaMockupOs): NebaMockupOs {
...
export function defaultNotch(device: NebaMockupDevice, os: NebaMockupOs): NebaMockupNotch {
  if (device !== 'mobile') {
    return 'none';
  }
 ...
 *
 * The ladder is `NebaSize`, and this is the second component after Box where
...
export const resolutions: Record<NebaMockupDevice, Record<NebaSize, NebaMockupResolution>> = {
  mobile: {
...
const shells: Record<NebaMockupDevice, Record<'thin' | 'standard' | 'thick', MockupShell>> = {
  mobile: {
...
const bareRadius: Record<NebaMockupDevice, number> = { mobile: 42, tablet: 24, desktop: 8 };
...
export const finishSlots: Record<NebaMockupFinish, React.CSSProperties> = {
  graphite: {
    '--n-shell': 'oklch(27% 0.008 262)',
    '--n-shell-edge': 'rgb(255 255 255 / 0.16)',
    '--n-shell-shade': 'oklch(17% 0.008 262)'
  } as React.CSSProperties,
  silver: {
    '--n-shell': 'oklch(83% 0.004 262)',
    '--n-shell-edge': 'rgb(255 255 255 / 0.75)',
    '--n-shell-shade': 'oklch(66% 0.006 262)'
  } as React.CSSProperties,
  white: {
    '--n-shell': 'oklch(96% 0.002 262)',
    '--n-shell-edge': 'rgb(255 255 255 / 0.9)',
    '--n-shell-shade': 'oklch(80% 0.004 262)'
  } as React.CSSProperties
};
...
export const shellClasses =
  'bg-(--n-shell) [box-shadow:inset_0_1px_0_var(--n-shell-edge),inset_0_0_0_1px_var(--n-shell-shade)]';
...
export const elevationFilters = [
  'none',
  'drop-shadow(0 1px 2px var(--neba-shadow-ambient))',
  'drop-shadow(0 2px 4px var(--neba-shadow-ambient)) drop-shadow(0 8px 12px var(--neba-shadow-ambient))',
  'drop-shadow(0 4px 6px var(--neba-shadow-ambient)) drop-shadow(0 18px 28px var(--neba-shadow-ambient))'
] as const;
...
export function MockupCutout({
  ...
  // A hook rather than a style, the same way `neba-link` and `neba-portal` are
 ...
 *
 * Two things about the fill, and both are deliberate. It is `--neba-surface`
 * rather than `--neba-panel` because a status bar that inherited the wallpaper
 * would be unreadable the moment somebody passed a dark one, and `--neba-surface`
 * is the token `--neba-fg` is guaranteed to contrast with in either theme. It is
 ...
 */
const barClasses =
  'relative z-10 flex shrink-0 items-center [background:color-mix(in_oklab,var(--neba-surface)_88%,transparent)]';

const barTextClasses = 'font-medium text-(--neba-fg) tabular-nums';
...
function Tile({
  ...
  const background =
    tone === 'accent'
      ? 'var(--n-accent)'
      : tone === 'faint'
        ? 'color-mix(in oklab, var(--neba-fg) 12%, transparent)'
        : 'color-mix(in oklab, var(--neba-fg) 26%, transparent)';
...
function StatusGlyphs({ height, gap }: { height: number; gap: number }) {
  return (
    <div className="flex items-center text-(--neba-fg)" style={{ gap }}>
...
function NavGlyphs({ size, gap }: { size: number; gap: number }) {
  return (
    <div
      className="flex items-center justify-center text-(--neba-fg)"
          ...
          >
            <div
              style={{
                ...
                height: 5,
                borderRadius: 3,
                background: 'color-mix(in oklab, var(--neba-fg) 55%, transparent)'
              }}
            />
          </div>
        )
    ...
    return {
      ...
      bottom: {
        ...
        node: (
          ...
          >
            <div
              className={`${barClasses} [box-shadow:var(--neba-shadow-2),var(--neba-plate-glass)]`}
            ...
            >
              ...
              <div
                style={{
                  width: 1,
                  height: 44,
                  background: 'color-mix(in oklab, var(--neba-fg) 16%, transparent)'
                }}
              />
    ...
    return {
      bottom: {
        ...
        node: (
          <div
            aria-hidden="true"
            className={`${barClasses} justify-center [border-block-start:1px_solid_var(--neba-border)]`}
            style={{ height: 52, paddingInline: 16 }}
          >
            <div className="flex items-center" style={{ gap: 10 }}>
              <div className="text-(--neba-fg)" style={{ opacity: 0.7 }}>
                <StartGlyph size={22} />
              </div>
  ...
  return {
    ...
    start: {
      ...
      node: (
        <div
          aria-hidden="true"
          className={`${barClasses} flex-col justify-start [border-inline-end:1px_solid_var(--neba-border)]`}
