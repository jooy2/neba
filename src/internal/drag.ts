 ...
 * property off the object and Safari goes on selecting text through the whole
 * drag. It is taken off the document rather than fixed with `preventDefault`,
 * which would also stop the browser focusing what was pressed.
 */
function takeSelection(): () => void {
  ...
  return () => {
    ...
    if (previous) body.style.setProperty('-webkit-user-select', previous);
...
export interface PointerDragOptions {
  ...
  onEnd?: () => void;
  /**
   * Whether the document's text selection is taken for the length of it.
   * @default true
   */
  selectable?: boolean;
  /**
   * Marks the target while it runs, for `data-[dragging]` to style against.
   * @default true
   */
  mark?: boolean;
}
