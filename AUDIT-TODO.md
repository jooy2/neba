...
- [ ] **194** **initials.ts** ~~slices by code point, so `'👩‍💻 Dev'` becomes `👩D` and `'🇰🇷 Team'` becomes `🇰T`~~ (fixed: `graphemesOf`). `toLocaleUpperCase()` has no locale, so the server and the browser may disagree (unverified; [decision], see H1). `src/internal/initials.ts:32-33`
