...
- [ ] **199** **HeatmapChart**: ~~the column label skip interval is computed per label, so "Mar 9" and "Mar 10" overlap~~ (fixed: one stride from the widest label). When all values are equal every cell takes the darkest step, so a week of zeros looks like peak activity ([decision]). `HeatmapChart.tsx:540-552`, `chart.ts:1473-1475`
