...
- [ ] **198** **PieChart**: ~~with `tooltip={false}` the arrow keys still change `active` and dim the other slices, with no status to read~~ (fixed: the keys do nothing with the tooltip off). Negative values become positive slices through `Math.abs`, while the treemap drops negatives ([decision]). `PieChart.tsx:152-156, 210, 287, 328-330`
