...
- **`nulls` on a `Sparkline` too.** The same three answers, and they matter more on a strip than on a chart: there is no tooltip and no axis to notice a hole with, so one that quietly closes over three missing weeks is a trend nobody can tell was interpolated. `zero` rewrites the values, so the range the strip scales itself to takes the nought in and the numbers read out to a screen reader change with it.
...
- **`brush` on every cartesian chart — a window on a series a plot cannot hold.** Two thousand points is a chart with no points on it: every column is a fraction of a pixel and the shape is a smear. The strip under the plot draws all of them small, the window on it says which part the chart draws, and dragging either handle or the window itself moves it. The handles are `role="slider"` buttons rather than shapes inside the picture, so the arrow keys move them and a screen reader is told which category each one sits on — the strip is a sibling of the plot for that reason, since `role="img"` makes everything under it presentational. It is drawn inside the chart's own height, like the axis labels. The window narrows the picture and nothing else: the hidden table and the exported file still hold every point.

- **`secondaryAxis`, and `axis: 'secondary'` on a series — two value scales on one plot.** For the series whose units are not the others': revenue beside a conversion rate, a count beside a duration. The second axis is drawn on the far edge — the right of a vertical chart, the top of one turned on its side — and its own `tickFormat` writes its series' numbers everywhere they appear, the ticks, the tooltip and the table, because the alternative is a percentage printed with the other axis' currency in the one place the two meet. It casts no gridlines: two grids on one plot is graph paper drawn twice, so its ticks land at the same count as the first axis' and the rules already there serve both. **Not read on a stacked chart** — a stack is a total, and a total across two units is not a number. Worth reaching for rarely, which is why the axis is drawn and labelled rather than merely implied: two scales let a caller make any two series look like they move together, and the reader cannot see that it was done.

- **`exportable` on every chart that has a table — the numbers as a CSV file.** A picture is the one form of a number nobody can paste anywhere, and the table a screen reader gets is not reachable with a pointer; this is the pointer's way to the same thing. `exportFileName` names the file and `onExport` takes the string instead of downloading it. The module that writes the CSV is **fetched on the press** rather than imported with the chart, which is `highlight.ts`' arrangement in miniature: a page that never turns this on downloads none of it, and no bundle moved. A `GaugeChart` does not take the three props at all — one reading is not a sheet — and the chart's own words gained an `exportCsv` entry in all nineteen languages.

- **`references` on every cartesian chart — lines and bands across the plot.** A target, an SLA, a budget, the window a forecast covers: the one thing on a chart that is neither a mark nor chrome. `value` places one and `to` turns it into a band; `axis: 'category'` reads the numbers against the other axis, for a rule that says _when_ rather than _how much_. The scale widens to hold them, so a target above everything measured is still on the chart, and one that names itself is read out with the data — the picture is `aria-hidden`, so a reader who gets the table instead would otherwise never learn the target exists. Drawn dashed and neutral unless told otherwise, because a reference in `danger` says the line is the bad thing when usually the bad thing is the data crossing it.

- **`xAxis` and `yAxis` on a `HeatmapChart`.** It had neither, so its column names could only be thinned by the stride — a grid of long stage names showed two of them and there was no prop to say otherwise. Both are **category** axes here, since the magnitude is on the colour ramp the scale legend describes, so `min`, `max`, `tickCount` and `grid` are not read; `label`, `tickFormat`, `hidden` and `thickness` are, and `xAxis.tickAngle` turns the column names. The arithmetic behind the turn moved into `chart.ts` as `turnedAxis`, so a heatmap and a cartesian chart on one dashboard cannot disagree about how deep a band of turned names runs.
...
- **`ContextWindow`** — how much of the context window has gone, and what went into it. A `Meter` draws the same reading as a bar and would do most of it; what it cannot do is the part that makes this a component: token counts are five and six digits long and have to be written compactly _in the reader's own language_, the split into input, output, reasoning and cached only means anything with the four next to each other, and the money underneath is a third unit again. `used` defaults to `input` plus `output` plus `reasoning`; `cached` is drawn as a row and never added, because every model that reports it means "this much of the input came out of a cache" and counting it twice would report the window as fuller than it is. A part that is `0` is still reported — a cache that returned nothing is a different fact from a model that has no cache.
...
- **An off `Switch` shows its thumb on the light theme.** The track drew the same `--neba-panel` every other resting surface does, which is white laid on the page — so it came out lighter than what it sits on, under a thumb that is `--neba-surface`, which is white. The disc that says where the switch is read at 1.02:1 against the track it runs along, and an off switch looked like an empty pill. The track now reads `--neba-groove`, a new token that darkens the page rather than brightening it, the way a groove does. Nothing moves on the dark theme, where the panel was already darker than the thumb and the groove is its two steps exactly.
