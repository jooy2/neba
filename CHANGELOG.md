...
- **A `Chip`'s count plate carries its own bed instead of another wash on top of the chip's.** `--n-on-tint` is solved for one wash, and the plate was `--n-soft-press` painted over whatever wash the chip already had: a count on a `text` chip read 4.2:1 and on a selected one 3.5:1. It is a fill now on every variant that is not itself filled, which holds 4.6:1 or better wherever the chip is sitting; a filled chip keeps the hole punched in its own fill. `Badge`, `Avatar` and `AppLogo` drop their `text` plate one step to `--n-soft-hover` for the same reason — they are small things a caller puts _inside_ other things, and one wash of headroom is what lets them land on a row that is already washed.
